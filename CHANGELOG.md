...
### 变更：`.说话` 指令改为 `.说 <内容>` 直接说出内容

- 指令由 `说话` 更名为 `说`。
- `说 <内容>`：用**当前配置音色**、走声音导演 LLM 改写（persona 注入）直接合成该内容并发语音；成功静默（语音即回复），失败写日志并在必要时兜底。
- 无内容（`说` 或 `说 `）：**静默取消**——不触发 TTS、不发提示。
- 原「武装下一条回复用语音（一次性）」行为移除；`use_voice` 工具的 force-voice 路径不受影响（仍在）。
